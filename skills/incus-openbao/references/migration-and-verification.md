...
When replacing an OpenBao system container with OCI, back up Raft/TLS and state, stop the old instance, verify the plan replaces the instance only, preserve the data volume, and unseal the new server with the original shares. Never bootstrap TLS or initialize against the preserved volume. Test a host-loss restore separately from instance replacement.
