...
Alternatively, if you have Docker, run the exact image version declared in
`authelia.tf` as a one-time command (it does not start the service):

```sh
docker run --rm -it authelia/authelia:4.39.28 authelia crypto hash generate argon2
```

Copy only the value after `Digest:` (beginning with `$argon2id# Homelab
